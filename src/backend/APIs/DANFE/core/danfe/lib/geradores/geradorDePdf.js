...
var pdfDefaults = {
    ...
    creditos: 'Lab41Dev - http://github.com/Lab41Dev'
};
...
module.exports = function(danfe, args, callback) {
    ...
    var emitente = danfe.getEmitente(),
        ...
        pdf = new Pdf({
            ...
            info: {
                ...
                Producer: 'http://github.com/Lab41Dev'
            }
        }),
    ...
    function desenharPagina() {
        ...
        linhaHorizontal(0, 0, 725); //0, 0, 751

        linhaVertical(alturaInicialDoQuadroDeItens, 725, 0);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 53.8);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 235.3);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 269.3);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 292);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 314.5);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 340.6); //QUI
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 371.2);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 405.4);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 439.3);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 473.2);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 507.2);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 535.6);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, 555.4);
        linhaVertical(alturaInicialDoQuadroDeItens, 725, larguraDoFormulario);

        var alturaInicialDoSetimoBloco = 793.6;

        //SEXTO BLOCO
        if(deveExibirQuadroDeCalculoDoIssqn) {
            linhaHorizontal(0, 0, 742.2);
            linhaHorizontal(0, 0, 762.2);

            linhaVertical(742.2, 762.2, 0);
            linhaVertical(742.2, 762.2, 136.1);
            linhaVertical(742.2, 762.2, (136.1 * 2) - 0.1);
            linhaVertical(742.2, 762.2, (136.1 * 3) - 0.1);
            linhaVertical(742.2, 762.2, larguraDoFormulario);
        } else {
            alturaInicialDoSetimoBloco = 762.2;
        }


        //MUDAR 821 PARA SUBIR ELEMENTO
        //288.25
        // SÉTIMO BLOCO
        linhaHorizontal(0, 0, alturaInicialDoSetimoBloco - 20); // x1, x2, y
        linhaHorizontal(0, 0, 821.8);

        linhaVertical(alturaInicialDoSetimoBloco-20, 821.8, 0);
        linhaVertical(alturaInicialDoSetimoBloco-20, 821.8, 388.25);
        linhaVertical(alturaInicialDoSetimoBloco-20, 821.8, larguraDoFormulario);
        ...
        //console.log("asdasdasdad", danfe._informacoesComplementares)
        ...
        //normal(string, x, y, largura, alinhamento, tamanho, _pdf)
        var textFont = 6
        
        if(informacoesComplementares.join(' - ').length <= 1000) textFont = 6//, console.log("DANGERRRR 5 ", informacoesComplementares.join(' - ').length)
        if(informacoesComplementares.join(' - ').length > 1000) textFont = 5//, console.log("DANGERRRR 4", informacoesComplementares.join(' - ').length)

        normal(informacoesComplementares.join(' - '), 1, alturaInicialDoSetimoBloco + -10.5, 386, 'justify', textFont);
        ...
        titulo('CNPJ / CPF', 392.5, 171, 192.5);
        ...
        titulo('DATA DA SAÍDA / ENTRADA', 495, 222, 90);
        ...
        titulo('HORA DA SAÍDA / ENTRADA', 495, 242, 90);
        ...
        if (transportador.getNome().length > 34 && transportador.getNome().length <= 45) {
            campo(transportador.getNome(), 1.5, 329, 166.5, 'left', 6);
        } else if (transportador.getNome().length <= 34) {
            campo(transportador.getNome(), 1.5, 329, 166.5, 'left', 7);
        } else {
            campo(transportador.getNome(), 1.5, 329, 166.5, 'left', 5);
        }
        ...
        campo(transportador.getEndereco().getPrimeiraLinha(), 1.5, 349, 254, 'rigth', args.tamanhoDaFonteDoCampo - 0.5);
        ...
        alturaDosTitulosDosItens = alturaDaSecaoDosItens + 20;

        if(numeroDePaginas > 1) {
            alturaDosTitulosDosItens = alturaDaSecaoDosItens + 30;
        }
        ...
        titulo('UN.', 320, alturaDosTitulosDosItens, 14.5, args.alinhamentoDoTituloDaTabela);
        titulo('QUANT.', 337, alturaDosTitulosDosItens, 37, args.alinhamentoDoTituloDaTabela);
        ...
        // SEXTO BLOCO
        if(deveExibirQuadroDeCalculoDoIssqn) {
            secao('CÁLCULO DO ISSQN', 1.5, 734.5);
            titulo('INSCRIÇÃO MUNICIPAL', 1.5, 744, 132.5);
            campo(emitente.getInscricaoMunicipal(), 1.5, 751, 132.5, 'left');

            titulo('VALOR TOTAL DOS SERVIÇOS', 137.5, 744, 132.5);
            campo(danfe.getValorTotalDosServicosFormatado(), 137.5, 751, 132.5, 'right');

            titulo('BASE DE CÁLCULO DO ISSQN', 273.5, 744, 132.5);
            campo(impostos.getBaseDeCalculoDoIssqnFormatada(), 273.5, 751, 132.5, 'right');

            titulo('VALOR TOTAL DO ISSQN', 409.5, 744, 175.5);
            campo(impostos.getValorTotalDoIssqnFormatado(), 409.5, 751, 175.5, 'right');
        }
        ...
        if(!deveExibirQuadroDeCalculoDoIssqn) {
            alturaDaSecaoDoSetimoBloco = 625;
        }

        alturaDosTitulosDoSetimoBloco = alturaDaSecaoDoSetimoBloco + 9;

        // SÉTIMO BLOCO
        secao('DADOS ADICIONAIS', 1.5, alturaDaSecaoDoSetimoBloco-20);
        titulo('INFORMAÇÕES COMPLEMENTARES', 1.5, alturaDosTitulosDoSetimoBloco-20, 385);
        titulo('RESERVADO AO FISCO', 390, alturaDosTitulosDoSetimoBloco-20, 195);
    ...
    itens.forEach(function(item) {
        function renderizarLinha(pdf) {
            ...
            normal(descricao, 55.5, yDoItemAtual, 178, 'center', args.tamanhoDaFonteDosItens, pdf);
            ...
            normal(item.getUnidade(), 315, yDoItemAtual, 25.5, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.getQuantidadeFormatada(), 332.5, yDoItemAtual, 46.5, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.getValorUnitarioFormatado(), 372, yDoItemAtual, 32.5, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.getValorTotalFormatado(), 407, yDoItemAtual, 31, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.getBaseDeCalculoDoIcmsFormatada(), 440, yDoItemAtual, 32.5, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.getBaseDeCalculoDoIcmsFormatada(), 440, yDoItemAtual, 32.5, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.getValorDoIcmsFormatado(), 474.5, yDoItemAtual, 32, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.getValorDoIpiFormatado(), 508.5, yDoItemAtual, 26, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
        ...
        if(alturaAtual + altura > (724 - yInicialDosItens)) {
             
            numeroDePaginas += 1;
            titulo('CÓDIGO x', 1.5, 300, 50.5, args.alinhamentoDoTituloDaTabela);
            pdf.addPage();
            yDoItemAtual = yInicialDosItens + 10;
            alturaAtual = 0;
