...
const NFe = require('./main_nfc/djf-nfe')
const TEMPLATE_DANFE = __dirname + "/template-danfe-nfe.hbs"
const fs = require('fs')
...
function getTemplateData(nfe) {
  ...
  var itensNfe = itens(nfe);
  var chave = formataChave(nfe.chave());
  itensNfe.forEach(function (item_nota, index) {
    dataextra.forEach(function (item_extra, index2) {
      if (item_nota.descricao == item_extra.descricao_item) {
       ...
       //mva_4
       if (item_extra.mva_4) {
         console.log("item_extra.mva_4 === ", item_extra.mva_4)
       ...
       */
        ...
        if (item_extra.aliquota == 17) {
          ...
          if (item_extra.calculo_valor_antecipado) {
            ...
            if(item_extra.tipo_antecipado == 'ISENTO'){
        ...
        item_extra.cfop_entrada ? item_nota.cfop_entrada = item_extra.cfop_entrada : item_nota.cfop_entrada = '';
