...
const { nfe, validacoes, dados, formatacoes, bancos, boleto } = require('../core/brasil/brazilian') //formatacoes.removerMascara
...
function NfeModel(content) {
  ...
  return {
    ...
    dest_inscricaoEstadual: function () {
      return value(xml.tagValue('dest').tagValue('IE'))
    },

    dest_nome: function () {
      return value(xml.tagValue('dest').tagValue('xNome'))
    },

    dest_cnpj: function () {
      return value((xml.tagValue('dest').tagValue('cnpj')))
    },

    dest_registroNacional: function () {
      return value((xml.tagValue('dest').tagValue('cnpj') || (xml.tagValue('dest').tagValue('cpf'))))
    },

    dest_fone: function () {
      return value(xml.tagValue('dest').tagValue('fone'))
    },

    dest_email: function () {
      return value(xml.tagValue('dest').tagValue('email'))
    },
    dest_cidade: function () {
      return value(xml.tagValue('dest').tagValue('xMun'))
    },
    dest_complemento: function () {
      return value(xml.tagValue('dest').tagValue('xCpl'))
    },

    dest_logradouro: function () {
      return value(xml.tagValue('enderDest').tagValue('xLgr'))
    },
    dest_numero: function () {
      return value(xml.tagValue('enderDest').tagValue('nro'))
    },

    dest_bairro: function () {
      return value(xml.tagValue('enderDest').tagValue('xBairro'))
    },

    dest_municipio: function () {
      return value(xml.tagValue('enderDest').tagValue('xMun'))
    },

    dest_cep: function () {
      return value(xml.tagValue('enderDest').tagValue('CEP'))
    },

    dest_uf: function () {
      return value(xml.tagValue('enderDest').tagValue('UF'))
    },
    ...
    inscricaoNacional: function () {
      var inscricaonacional = this.cnpj() || this.cpf()

      if(inscricaonacional.length == 14){
        //É um cnpj
        inscricaonacional = formatacoes.cnpj(inscricaonacional) 
    }
    if(inscricaonacional.length == 11){
        //É um CPF
        inscricaonacional = formatacoes.cpf(inscricaonacional) 
    }

      return value(inscricaonacional || '' )
    },
    ...
    descricaoTeste:function (nrd) {
      return value(xmlModel(xml.tagGroup('xProd')[nr]))
    },
    ...
    porcentagemIcmsST: function () {
      return value(xml.tagValue('pICMSST'))
    },

    porcentagemIcms: function () {
    ...
    valorTotaldosProdutos : function (){
      return value(xml.tagValue('total').tagValue('ICMSTot').tagValue('vProd'))
    },
    ...
    transporte_nome: function () {
      return value(xml.tagValue('transp').tagValue('xNome'))
    },

    transporte_ufplaca: function () {
      return value(xml.tagValue('transp').tagValue('xNome'))
    },
    ...
    modalidadeFreteTexto: function () {
      var modalidades = {
        0: 'porContaDoEmitente',
        1: 'porContaDoDestinatarioRemetente',
        2: 'porContaDeTerceiros',
        9: 'semFrete'
      }
    ...
    valorImpostodeImportacao: function () {
